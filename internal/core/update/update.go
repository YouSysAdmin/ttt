// Package update checks GitHub releases for a newer ttt and can replace the
// running binary in place. Package-managed installs (apk/deb/rpm/brew) are
// detected in source.go and must be upgraded through their package manager
// instead.
package update
...
const (
	...
	RepoURL = "https://github.com/YouSysAdmin/ttt"

	binaryName = "ttt"
)

// ReleaseAPIURL is the GitHub API endpoint for the latest release. A var so
// tests can point it at a local server.
var ReleaseAPIURL = "https://api.github.com/repos/YouSysAdmin/ttt/releases/latest"
...
// isDev reports whether version identifies an unreleased build. The Makefile
// stamps main.version, which defaults to "dev" when built by hand.
...
// given version, OS, and architecture, matching the .goreleaser.yml
// name_template.
...
func fetchRelease(apiURL string) (Release, error) {
	return fetchReleaseWith(httpClient, apiURL)
}

func fetchReleaseWith(client *http.Client, apiURL string) (Release, error) {
...
func downloadFile(url string) (string, error) {
	...
	tmp, err := os.CreateTemp("", "ttt-update-*")
	if err != nil {
		return "", err
	}
	defer tmp.Close()
...
func extractBinary(archivePath, binaryName string) ([]byte, error) {
	if strings.HasSuffix(archivePath, ".zip") || runtime.GOOS == "windows" {
		if data, err := extractZip(archivePath, binaryName); err == nil {
			return data, nil
		}
	}
...
// execPath returns the resolved path of the currently running binary.
// A var so tests can substitute a temp file.
